...
namespace CrowdTagMovie.Controllers.Api
{
    ...
    public class DrinkController : ApiController
    {
		...
		public IHttpActionResult UpdateTagCategory(int categoryId, [FromBody]TagCategoryDTO dto)
		{
			...
			dto.ID = categoryId;
			UoW.TagCategoryRepository.Update(dto);
			UoW.Commit();
		...
		public IHttpActionResult CreateNewTag(int categoryId, [FromBody]TagDTO dto)
		{
			...
			var entity = new Tag();
			dto.UpdateEntity(ref entity);
			// BOOKMARK: Set the CategoryID on the Tag
		...
		public IHttpActionResult UpdateTag(int tagId, [FromBody]TagDTO dto)
		{
			if (!ModelState.IsValid) return BadRequest(ModelState);

			dto.ID = tagId;
			UoW.TagRepository.Update(dto);
			UoW.Commit();
